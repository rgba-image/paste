import { SourceToDest, rgbaToUint32, isLittleEndian } from '@rgba-image/common'

export const paste: SourceToDest = ( source: ImageData, dest: ImageData, sx = 0, sy = 0, sw = source.width - sx, sh = source.height - sy, dx = 0, dy = 0 ) => {
  sx = sx | 0
  sy = sy | 0
  sw = sw | 0
  sh = sh | 0
  ...
  for ( let y = 0; y < sh; y++ ) {
    for ( let x = 0; x < sw; x++ ) {
      ...
      if ( destIndex >= dest.data.length ) return
      ...
      const v = rgbaToUint32( r * 255, g * 255, b * 255, a * 255, isLittleEndian )
