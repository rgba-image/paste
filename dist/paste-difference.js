...
exports.pasteDifference = void 0;
const common_1 = require("@rgba-image/common");
const pasteDifference = (source, dest, sx = 0, sy = 0, sw = source.width - sx, sh = source.height - sy, dx = 0, dy = 0, sourceAlpha = 1) => {
    sx = sx | 0;
    sy = sy | 0;
    sw = sw | 0;
    sh = sh | 0;
    ...
    for (let y = 0; y < sh; y++) {
        ...
        for (let x = 0; x < sw; x++) {
            ...
            const dba = dB * dA;
            const a = dA + sA - dA * sA;
            const r = (sra + dra - 2 * Math.min(sra * dA, dra * sA)) / a;
            const g = (sga + dga - 2 * Math.min(sga * dA, dga * sA)) / a;
            const b = (sba + dba - 2 * Math.min(sba * dA, dba * sA)) / a;
            //
...
exports.pasteDifference = pasteDifference;
//# sourceMappingURL=paste-difference.js.map