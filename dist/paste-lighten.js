...
exports.pasteLighten = void 0;
const common_1 = require("@rgba-image/common");
const pasteLighten = (source, dest, sx = 0, sy = 0, sw = source.width - sx, sh = source.height - sy, dx = 0, dy = 0, sourceAlpha = 1) => {
    sx = sx | 0;
    sy = sy | 0;
    sw = sw | 0;
    sh = sh | 0;
    ...
    for (let y = 0; y < sh; y++) {
        ...
        for (let x = 0; x < sw; x++) {
            ...
            const dba = dB * dA;
            const a = dA + sA - dA * sA;
            const r = (Math.min(sra * dA, dra * sA) +
                sra * (1 - dA) +
                dra * (1 - sA)) / a;
            const g = (Math.min(sga * dA, dga * sA) +
                sga * (1 - dA) +
                dga * (1 - sA)) / a;
            const b = (Math.min(sba * dA, dba * sA) +
                sba * (1 - dA) +
                dba * (1 - sA)) / a;
            //
...
exports.pasteLighten = pasteLighten;
//# sourceMappingURL=paste-lighten.js.map