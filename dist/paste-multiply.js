...
exports.pasteMultiply = void 0;
const common_1 = require("@rgba-image/common");
const pasteMultiply = (source, dest, sx = 0, sy = 0, sw = source.width - sx, sh = source.height - sy, dx = 0, dy = 0, sourceAlpha = 1) => {
    sx = sx | 0;
    sy = sy | 0;
    sw = sw | 0;
    sh = sh | 0;
    ...
    for (let y = 0; y < sh; y++) {
        ...
        for (let x = 0; x < sw; x++) {
            ...
            const dba = dB * dA;
            const a = dA + sA - dA * sA;
            const r = (sra * dra + sra * (1 - dA) + dra * (1 - sA)) / a;
            const g = (sga * dga + sga * (1 - dA) + dga * (1 - sA)) / a;
            const b = (sba * dba + sba * (1 - dA) + dba * (1 - sA)) / a;
            //
...
exports.pasteMultiply = pasteMultiply;
//# sourceMappingURL=paste-multiply.js.map